...
const appManifest=read('./app/manifest.ts');
const appNavigation=read('./src/features/navigation/app-nav.tsx');
const householdRevision=read('./server/household-revision.ts');
const householdRevisionClient=read('./src/features/realtime/use-household-revision.ts');
...
assert.equal(tooSmallFonts.length,0,'Readable UI text must not fall below 12px.');
assert.ok(!appNavigation.includes("href:'/accounts'"),'Accounts should not compete for a mobile bottom-nav slot.');
assert.match(appManifest,/share_target/);
assert.match(serviceWorker,/\/share-target/);
assert.match(serviceWorker,/nestbalance-share-v1/);
assert.match(householdRevision,/auditEvents/);
...
assert.match(localImageOcr,/por/);
assert.match(localImageOcr,/eng/);
assert.match(localImageOcr,/spa/);
...
assert.match(productionWorkflow,/\.release == \$sha/);
...
assert.match(officialProductionWorkflow,/\.release == \$sha/);
