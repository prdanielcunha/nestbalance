  ...
  async function confirm() {
    ...
    try {
      ...
      for (let i = 0; i < interpretations.length; i++) {
        const result = await commitInterpretation({ householdId, uid, interpretation: interpretations[i], file: i === 0 ? file : null });
      ...
      setOpen(false); setText(''); setFile(null); setInterpretations([]); onCommitted?.();
    } catch { setError('Não conseguimos salvar isso agora. Tente novamente.'); }
    ...
    {open && <div className="sheet-backdrop" role="presentation" onMouseDown={e => e.target === e.currentTarget && !saving && setOpen(false)}>
      <section className="capture-sheet" role="dialog" aria-modal="true" aria-label={t.captureTitle}>
        {!interpretations.length ? <>
          ...
          <div className="sheet-actions"><button className="ghost-button" onClick={()=>setOpen(false)}>{t.cancel}</button><button className="primary-button" disabled={!text.trim()} onClick={interpret}>Entender</button></div>
        </> : <>
          ...
          {error && <p className="error-copy" role="alert">{error}</p>}
          <div className="sheet-actions"><button className="ghost-button" disabled={saving} onClick={()=>setInterpretations([])}>{t.cancel}</button><button className="primary-button" disabled={saving} onClick={confirm}>{saving ? 'Guardando…' : t.confirm}</button></div>
        </>}
      </section>
    </div>}
  </>;
