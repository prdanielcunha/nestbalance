    ...
    {loading?<div className="together-loading" role="status"/>:<>
      ...
      <div className="together-grid">
        ...
        <section className="together-panel">
          ...
          {canContribute&&<div className="comment-composer"><textarea value={comment} onChange={e=>setComment(e.target.value)} maxLength={800} rows={3} placeholder={l('Registre uma decisão, dúvida ou combinado…','Record a decision, question, or agreement…','Registra una decisión, duda o acuerdo…')}/><div><select value={mentionUid} onChange={e=>setMentionUid(e.target.value)}><option value="">{l('Sem menção','No mention','Sin mención')}</option>{members.filter(item=>item.uid!==uid).map(member=><option key={member.uid} value={member.uid}>{nameFor(member.uid)}</option>)}</select><button disabled={working||!comment.trim()} onClick={()=>void addComment()}>{l('Enviar','Send','Enviar')}</button></div></div>}
        </section>
      </div>
