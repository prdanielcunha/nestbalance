...
export function AssistantScreen({householdId}:{householdId:string}){
  ...
  async function ask(value?:string){
    ...
    setError('');
    try{
      const response=await askFinanceAssistant(householdId,text);
      setResult(response);
    }catch{
...
<Link href="/household" className="avatar-dot" aria-label="Lar e acessos" />
    ...
    <section className="assistant-hero">
      ...
      <p>O Assistente responde somente com o que o NestBalance consegue comprovar no seu Lar. Quando não souber, ele diz que não sabe.</p>
