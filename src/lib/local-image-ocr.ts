...
export type LocalOcrLocale='pt-BR'|'en'|'es';

export function localOcrLanguage(locale:LocalOcrLocale='pt-BR'){
  return locale==='en'?'eng':locale==='es'?'spa':'por';
}

export async function readImageTextLocally(
  file:File,
  onProgress?:(progress:LocalOcrProgress)=>void,
  locale:LocalOcrLocale='pt-BR'
):Promise<string>{
  ...
  const worker=await createWorker(localOcrLanguage(locale),OEM.LSTM_ONLY,{
    logger(message:any){
